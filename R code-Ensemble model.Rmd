...
```{r}
library(xlsx)
star<- read.csv("test_set_with_predictions-The ensemble model.csv")
...
```{r}
tiff(filename="Decision_curve_Ensemble Prediction.tif", width=600*6, height=600*6, res=90*6)
par(mfrow=c(1,1), mar=c(4.2,4.2,3,.5), pty="m")
plot_decision_curve(modelt,curve.name=c('Ensemble Prediction'),xlim=c(0,0.8),ylim=c(0,0.2),
...
tiff(filename="Calibration_curve-Ensemble Prediction.tif", width=600*6, height=600*6, res=90*6)
plot(cali,
     xlim=c(0,1),
     ylim=c(0,1),
...
legend(0.55,0.35,
       ...
       bty="n")
title(paste("Ensemble Prediction"))
dev.off()
```
...
tiff(filename="xiangshitu-Ensemble Prediction.tif", width=800*6, height=600*6, res=90*6)
...
ggplot(data=star,aes(x = anxietyanddepression, y =lr_pred_proba)) +
  ...
  scale_fill_manual(values=c("green", "red"))+
    ggtitle(paste("Ensemble Prediction, Discrimination Slope=", 
